...
int main(){
    ...
    for (int i = 0; i < 19; i++){
        cudaMemcpy(d_arrC, arrCC, 2 * sizeof(float), cudaMemcpyHostToDevice);
        add<<<120, 256>>>(d_arrC, arrGpuMan, n);
        cudaDeviceSynchronize();
        cudaMemcpy(arrCC, d_arrC, sizeof(float) * 2, cudaMemcpyDeviceToHost);
        dummyMan = arrCC[1];
    }
    ...
    for (int i = 0; i < 19; i++){
        cudaMemPrefetchAsync(arrGpuUniMan, sizeof(float) * 2, 0);
        cudaDeviceSynchronize();


        add<<<120, 256>>>(arrGpuUniMan, arrGpuUniManRes, n);
        cudaMemPrefetchAsync(arrGpuUniMan, sizeof(float) * 2, cudaCpuDeviceId);
        cudaDeviceSynchronize();
        dummyUniMan = arrGpuUniMan[1];
    }
